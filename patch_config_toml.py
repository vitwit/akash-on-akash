...
peers_input = str(os.environ.get('PEERS',''))
seeds_input = str(os.environ.get('SEEDS',''))
seed_mode = int(os.environ.get('SEED_MODE', '0')) > 0

seeds_url = os.environ.get('SEEDS_URL','')
sync_url = os.environ.get('SYNC_URL','')
cli_name = os.environ.get('CLI_NAME','sentinelhubcli')
...
data['p2p']['seeds'] = seeds_str if seeds_str else seeds_input
data['p2p']['persistent_peers'] = seeds_str if seeds_str else peers_input
data['p2p']['pex'] = pex_enable
data['p2p']['private_peer_ids'] = private_peers
data['p2p']['addr_book_strict'] = addr_book_strict
data['p2p']['seed_mode'] = seed_mode
